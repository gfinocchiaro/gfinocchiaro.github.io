...
What Is MQTT.Cool? {#_what_is_mqtt_cool}
------------------
...
### Architecture Extensions {#_architecture_extensions}
...
### Performance Extensions {#_performance_extensions}
...
### Security Extensions {#_security_extensions}
...
MQTT.Cool Architecture {#_mqtt_cool_architecture}
----------------------
...
> **Tip**
>
> Base knowledge of the MQTT protocol is required to fully understand
> this guide.
...
### Check System Requirements {#_check_system_requirements}
...
### Download and Install {#_download_and_install}
...
### Attach an MQTT Broker {#_attach_an_mqtt_broker}
...
> **Note**
>
> This example is a *light* version of the [Hello IoT
> World](https://github.com/MQTTCool/MQTT.Cool-example-Hello_IoT_World-client-javascript)
> example available on GitHub, where also graphical gauges are employed.

### Step 1: Setup an MQTT Broker {#_step_1_setup_an_mqtt_broker}
...
> **Note**
>
> For the purpose of the tutorial, there is no requirement to use a
> specific MQTT broker, therefore feel free to use your preferred MQTT
> broker.
...
> **Tip**
>
> If you are familiar with Docker, you might also want to install and
> run the [official
> image](https://store.docker.com/images/eclipse-mosquitto). Please also
> consider that you could use one of the public test brokers available
> for development and testing purposes:
> [here](https://github.com/mqtt/mqtt.github.io/wiki/public_brokers) an
> up-to-date list.

### Step 2: Attach the MQTT Broker {#_step_2_attach_the_mqtt_broker}
...
provided in [section\_title](#static_lookup).
...
### Step 3: Write the Feed Simulator {#_step_3_write_the_feed_simulator}
...
-   Create a new client instanced targeted to the MQTT broker running on
    localhost and listening on TCP port 1883; the client is identified
    as `telemetry-feed`.

-   Connect and wait for response.

-   Once connected, schedule a task for generating and publishing
    telemetry data every 100 ms.
...
> **Tip**
>
> As shown by the full example on GitHub, using a dependency management
> tool like Maven is a more practical way to include the Paho Java
> Client in your projects.

### Step 4: Make the JavaScript Client {#_step_4_make_the_javascript_client}
...
-   Load the MQTT.Cool JavaScript library file from the
    [`unpkg`](https://unpkg.com/#/) service.

-   Connect to the local MQTT.Cool server, listening on TPC port `8080`.

-   On successful connection, make a new
    [`MqttClient`](http://www.lightstreamer.com/api/mqtt.cool-web-client/latest/MqttClient.html)
    instance, ready to be connected to the Mosquitto server as
    configured in `mqtt_master_connector_conf.xml`.
...
-   Once a new message arrives, elaborate its topic name in order to
    retrieve the identifier relative to the `<div>` element to be
    updated.
...
Addressing MQTT Brokers {#_addressing_mqtt_brokers}
=======================
...
-   **&lt;connection\_param\_J&gt;** is the J\_th connection parameter
    ...
    -   `clientid_prefix`, the Client Id prefix to be used for *shared
        connections*, as will be detailed in
        [section\_title](#shared_connection)
...
> **Tip**
>
> See [appendix\_title](#_connection_parameters) for a more in-depth
...
### The Hook Variant {#static_lookup_hook}
...
detailed in [section\_title](#hook_broker_configuration): when no
configurations exist in `mqtt_master_connector_conf.xml`, MQTT.Cool will
ask the Hook to supply a valid one.

### Client Side Settings {#_client_side_settings}
...
### Final Considerations {#_final_considerations}
...
### Client Side Settings {#_client_side_settings_2}
...
### Final Considerations {#_final_considerations_2}
...
Client Application Development {#_client_application_development}
==============================
...
### Deployment Architecture {#_deployment_architecture}
...
SDK for Web Clients {#_sdk_for_web_clients}
-------------------
...
> **Tip**
>
> Alternatively, since `unpkg` offers a free CDN with no guarantee on
> the quality of service, you might want to host the library yourself or
> host it on another CDN. To do this, simply download the library from
> <https://unpkg.com/mqtt.cool-web-client/dist/mqtt.cool.js>.

### The UMD Pattern {#_the_umd_pattern}
...
> **Tip**
>
> Starting from [section\_title](#connection_pattern), you will be
> provided with full details of these concepts.
...
SDK for Node.js Clients {#_sdk_for_node_js_clients}
-----------------------
...
> **Note**
>
> The additional `--save` flag will update your `package.json` (if any)
...
The MQTT.Cool Connection Pattern {#connection_pattern}
--------------------------------
...
-   Open a new session against the MQTT.Cool server running at the
    specified address.

-   Upon successful connection, the `onConnectionSuccess` callback
    function is invoked.

-   Through the `mqttCoolSession` parameter (which implements the
    [`MQTTCoolSession`](http://www.lightstreamer.com/api/mqtt.cool-web-client/latest/MQTTCoolSession.html)
    interface), create a new
    [`MqttClient`](http://www.lightstreamer.com/api/mqtt.cool-web-client/latest/MqttClient.html)
    instance that is configured to connect to the MQTT broker aliased by
    *mybroker*, using *myclientid* as client identifier.
...
-   You can open a session to the MQTT.Cool server also specifying
    ...
    any (as will be detailed in
    [section\_title](#hook_authorizing_session)). For example:
    ...
    >
    > See the
    > [`openSession`](http://www.lightstreamer.com/api/mqtt.cool-web-client/latest/global.html#openSession)
    > API reference for a complete description of all allowed invocation
    > forms.

-   As already mentioned in [section\_title](#static_lookup), normally
    you will use an alias to statically lookup the connection settings
    relative to the MQTT broker to connect to; but you could also lean
    on the Dynamic Lookup (see [section\_title](#dynamic_lookup)) to
    bypass any static configuration and connect to the supplied URI, as
...
The MQTTCoolSession Interface {#session_interface}
-----------------------------
...
``` {#main_connection_pattern_ext .javascript}
mqttcool.openSession(..., {
...
-   [`onLsClient`](http://www.lightstreamer.com/api/mqtt.cool-web-client/latest/MQTTCoolListener.html#onLsClient)
    ...
    MQTT.Cool (see
    [appendix\_title](#_access_the_lightstreamer_client_api)).
...
> **Note**
>
> Connection failures may also include any authorization issue raised by
> the plugged Hook, as will be discussed in
> [section\_title](#hook_authorizing_session).

The MqttClient Interface {#_the_mqttclient_interface}
------------------------
...
### Multiplexed MQTT Channels {#_multiplexed_mqtt_channels}
...
### Dedicated End-to-End Connection {#dedicated_connection}
...
> **Note**
>
> The MQTT.Cool server does not take any active roles in the management
> of the Session State, which is, on the contrary, maintained
> exclusively by the two ends of the connection, the MQTT broker and
> `MqttClient`.
...
> **Note**
>
> Any issue on the MQTT.Cool connection detected on the client side will
> cause the `MqttClient` instance to try a reconnection (as you will see
> in [section\_title](#reconnecting)).

### Shared End-to-End Connection {#shared_connection}
...
> **Note**
>
> While joining them into shared broker connections, the MQTT.Cool
> server takes in considerations only activated MQTT channels,
> regardless of the MQTT.Cool connections on the client side they have
> been multiplexed into.
...
Specifying Connection Options {#connection_options}
-----------------------------
...
> **Warning**
>
> The credentials used to connect to MQTT brokers are **not** related to
> the ones which may be specified to open new sessions against
> MQTT.Cool.
...
-   [`onNotAuthorized`](http://www.lightstreamer.com/api/mqtt.cool-web-client/latest/global.html#OnConnectionNotAuthorized)
    ...
    Hook, if any (as will be shown in
    [section\_title](#hook_authorizing_connection)).
...
> **Note**
>
> Will Message and Clean Session flag can only be set in the context of
> a **dedicated connection**

### Managing Persistent Session {#_managing_persistent_session}
...
> **Note**
>
> In the case of Node.js, the additional `storePath` property allows you
> to specify the directory which will host the files used to store the
> session data:
>
> ``` {.javascript}
...
### Managing Connection Lost {#managing_connection_lost}
...
-   Stop of the reconnection attempts which follow an interruption of
    the MQTT.Cool connection, as will be discussed in
    [section\_title](#starting_to_reconnect).
...
The Message Class {#_the_message_class}
-----------------
...
> **Tip**
>
> `destinationName`, `qos` and `retained` are defined as read-write
> properties, while in contrast `duplicate` is a **read-only** property
> as it can only be set on messages received from the broker.
...
> **Warning**
>
> You cannot provide arguments in a mixed form (for example, passing a
> `Message` instance as first parameter and a topic as second).

### Message Delivery Notification {#message_delivery_notifications}
...
described in [section\_title](#hook_authorizing_publishing)):
...
> **Note**
>
> The callback will be never invoked in case of any authorization issue
> raised by the plugged Hook. Under this circumstance, the message is
> also removed from the local storage if session persistence is active.
...
### Subscribe Options {#subscribe_options}
...
could be raised by the plugged Hook (as you will see in
[section\_title](#hook_authorizing_subscription)):
...
### Message Arriving Notification {#_message_arriving_notification}
...
> **Warning**
>
> The `onFailure` callback function is not actually employed by the
> current implementation of the library, as the MQTT Protocol
> Specifications does not define any behaviors in case of unsubscription
> failure. As a consequence of this, such function is only formally
> provided, but future use cannot be ruled out.
...
### The MqttClient Session State {#_the_mqttclient_session_state}
...
> **Warning**
>
> The callback is invoked only if the `MqttClient` instance was
> previously in the connected status: if the MQTT.Cool connection was
> interrupted just before `MqttClient` connects through the `connect`
> method, a connection failure will be immediately triggered. See the
> API reference relative to the
> [OnConnectionFailure](http://www.lightstreamer.com/api/mqtt.cool-web-client/latest/global.html#OnConnectionFailure)
> (the case with `errorCode=10`).

### Completing Reconnection {#_completing_reconnection}
...
[`onConnectionLost`](http://www.lightstreamer.com/api/mqtt.cool-web-client/latest/MqttClient.html#onConnectionLost)
to be fired (as already anticipated in
[section\_title](#managing_connection_lost)). However, the MQTT.Cool
...
> **Tip**
>
> From the disconnected status, an `MqttClient` object is allowed to
> open a new connection through the
> [`connect`](http://www.lightstreamer.com/api/mqtt.cool-web-client/latest/MqttClient.html#connect)
> method.

The MQTT.Cool Hook {#_the_mqtt_cool_hook}
==================
...
Hook Development {#_hook_development}
----------------

The following sections will guide you through the core concepts of Hook
development.

### Setting Up the Development Environment {#_setting_up_the_development_environment}
...
> **Note**
>
> Please note that other dependency management tools like *Gradle* or
> *Ivy* may be used as well. Take a look at the
> `<MQTT.COOL_HOME>/DOCS-SDKs/sdk_hook/README.TXT` file (also available
> [online](http://www.lightstreamer.com/latest/mqtt.cool/DOCS-SDKs/sdk_hook/README.TXT))
> for more information.

### The MQTTCoolHook Interface {#_the_mqttcoolhook_interface}
...
### Notifying of Hook Initialization {#_notifying_of_hook_initialization}
...
> **Warning**
>
> Any exception thrown from the method will cause the failure of the
> initialization phase and, as a consequence of this, the MQTT.Cool
> server process will abort.

### Authorizing Session Opening {#hook_authorizing_session}

If you want to intercept a new session started by the client (as
detailed in [section\_title](#connection_pattern)), you have to override
the `canOpenSession` method:
...
As you have seen in [section\_title](#session_interface), the
[`onConnectionFailure`](http://www.lightstreamer.com/api/mqtt.cool-web-client/latest/MQTTCoolListener.html#onConnectionFailure)
...
> **Note**
>
> As you will see for the next operations, `sessionId` is always
> included in the signature of all Hook methods as the first parameter
> (except `init` and `resolveAlias`): it will take on the meaning of
> **already trusted session**, hence there will be no chances that any
> subsequent Authorization Request or *Simple Notification* may be
> invoked as part of an *untrusted session*.
...
> **Note**
>
> In the case of shared connection, although the physical MQTT
> connection between the MQTT.Cool server and the target MQTT broker is
> realized only the very first time that a client requests to connect,
> the `canConnect` method is invoked on each connection request issued
> by every *joining* client.
...
[section\_title](#connection_options):
...
![Sequence diagram for connection
...
As already anticipated in [section\_title](#static_lookup_hook), you can
program your custom Hook to provide a valid MQTT broker configuration
for a specific connection alias, when no other static entries have been
supplied in the `mqtt_master_connector_conf.xml` file for the same
alias. All you need is to override the `resolveAlias` method, where you
will provide an instance of the
...
> **Note**
>
> Through `MqttBrokerConfig` you can set all the same configuration
> parameters already discussed in [section\_title](#static_lookup).
...
As anticipated in [section\_title](#message_delivery_notifications), on
the client side you can leverage the
[`onMessageNotAuthorized`](http://www.lightstreamer.com/api/mqtt.cool-web-client/latest/MqttClient.html#onMessageNotAuthorized)
...
[`onSubscriptionNotAuthorized`](http://www.lightstreamer.com/api/mqtt.cool-web-client/latest/global.html#OnSubscriptionNotAuthorized)
callback function, as introduced in
[section\_title](#subscribe_options):
...
explicitly closed by the originating client (see
[section\_title](#disconnecting)); you will be informed of this event
...
> **Warning**
>
> `onDisconnection` is also invoked because of session interruption
> while the client is currently connected: if it is the case, both
> `onSessionClose` and `onDisconnection` will be triggered.

Lastly, a currently active subscription may be terminated by an
unsubscription request (see [section\_title](#unsubscribing)), which
will be reported by the `onUnsubscribe` method:
...
Packaging, Configuration and Deployment {#_packaging_configuration_and_deployment}
---------------------------------------
...
3.  Edit
    ...
    > **Tip**
    >
    > Alternatively, you might want to uncomment and replace the
    ...
    > <!--
    > <param name="hook">my.package.Hook</param>
    > -->
    > ```
...
Connection Parameters {#_connection_parameters}
=====================
...
> **Tip**
>
> The details below are similar to the inline documentation provided in
> the same file.
...
:   (Optional). Client Id prefix to be used for shared connections. If
    the clients want to share a single connection (see
    [section\_title](#shared_connection)), a randomly generated suffix
    will be appended in order to generate a unique ClientId, for opening
    the physical connection toward the MQTT broker. Uniqueness of the
    ClientId is mandatory as multiple shared connections may exist for
    the same broker.
...
Access the Lightstreamer Client API {#_access_the_lightstreamer_client_api}
===================================
...
The LightstreamerClient Object {#_the_lightstreamerclient_object}
------------------------------

In [section\_title](#session_interface), we have briefly mentioned the
[`onLsClient`](http://www.lightstreamer.com/api/mqtt.cool-web-client/latest/MQTTCoolListener.html#onLsClient)
...
### Managing the Connection Options {#_managing_the_connection_options}
...
> **Warning**
>
> Changes made to the `ConnectionOptions` object will affect only the
> connection established between the client and the MQTT.Cool server.
> The connection parameters relative to MQTT brokers have been already
> discussed in [Addressing MQTT Brokers](#_addressing_mqtt_brokers) and
> [appendix\_title](#_connection_parameters).
...
Bandwidth Throttling {#_bandwidth_throttling}
--------------------

In [section\_title](#_frequency_management) we have seen how MQTT.Cool
allows specifying the rate at which messages can arrive for a specific
MQTT topic. In addition, the underlying Lightstreamer Engine permits to
go beyond that: you can also specify the maximum bandwidth allowed for
all messages routed by the MQTT.Cool server for this specific session.
...
> The [Hello IoT
> World](https://github.com/MQTTCool/MQTT.Cool-example-Hello_IoT_World-client-javascript)
> example available on GitHub employs the same technique to issue a new
> max bandwidth request according to the movements on the slider, as
> shown in
> [index.js](https://github.com/MQTTCool/MQTT.Cool-example-Hello_IoT_World-client-javascript/blob/master/src/web/js/index.js)
> (line 112).

Attaching a Listener {#_attaching_a_listener}
--------------------
...
Private Operations {#_private_operations}
------------------
